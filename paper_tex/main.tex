...
\paragraph{Why K=5 on the preservation side adds nothing (conjecture, scoped to the eval distributions we used).}
...
(half-spread 0.018 vs \code{aug\_kl\_k1}'s 0.006). \textbf{Important
scoping caveat:} the K=1 KL anchor in \code{aug\_kl\_k1} uses the
Original QD-decomposer framing only, and our preservation R@10 eval also
uses the Original framing -- so the (e) $\approx$ (d) null on absorption
F1 strictly tells us ``K=5 KL is redundant \emph{for the Original
framing, the canonical task-tuned format and the deployment target}.''
Whether \code{aug\_kl\_k1} silently degrades the four non-Original
framings (Bare, Analyst, Detailed, Request) is not measured in our
current eval suite. Since the Original framing is what production
deployments would actually hit, this is the load-bearing measurement;
but a held-out R@10 sweep across all 5 framings on the (d) and (e)
round-15 checkpoints ($\sim$30 min on existing artifacts) would
distinguish ``shared subspace makes K=5 KL truly redundant'' from
``K=1 protects only Original at the cost of unmeasured drift on the
other 4 framings.'' We flag this as a follow-up. At larger model
scales, longer round chains, or with adversarially chosen preservation
framings, format-selective forgetting may become detectable; we do not
claim the symmetric construction is uniformly worthless, only that at
4B/$r=16$/200-edits-per-round/15-rounds and on Original-framing
absorption + preservation, the simpler \code{aug\_kl\_k1} is empirically
the right method.
