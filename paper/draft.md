...
Second, the paper diagnoses the **absorption-integration gap** using two complementary probes. A LoRA-delta subspace analysis (Phase 6) shows all four COPR variants share a near-identical top input direction (first-principal-angle cosine 0.997 pairwise) and differ mainly in total Frobenius norm (`copr_gold_injection_anchored` highest at 1.472) and stable rank. A hidden-state geometry probe (Phase 7) shows every method shifts the QA-direction representation substantially more than the QD-direction representation for the same subject, with plain `copr` and `copr_anchored` actively anti-aligning (cos drops -0.14 and -0.10). A behavioral QD-format probe (Phase 7b) then measures directly whether the injected fact is accessible when the same probe question is wrapped in the downstream task's system prompt: every method exhibits a format gap (QA F1 minus QD F1) of 0.05-0.14, with gold-injection variants showing the *largest* behavioral gap (+0.12 to +0.14) despite the *smallest* geometric shift ratio --- a disagreement we take as evidence that cosine proximity is not a reliable proxy for behavioral integration.
...
Fourth, the paper shows **V-REx-style variance penalization beats the strongest baseline (15-round KL-regularized SFT) at matched compute on every measured axis except preservation**. Trained for 15 chained sequential rounds against 15 rounds of KL-regularized SFT at matched compute, FI-SFT delivers: absorption F1 0.140 vs 0.118 (+19%), fact-mean F1 0.158 vs 0.118 (+34%), fact-worst F1 0.120 vs 0.090 (+33%), locality overall F1 0.060 vs 0.048 (+25%), behavioral QD-format F1 0.092 vs 0.072 (+29%), and format gap 0.037 vs 0.072 (cut nearly in half). Preservation trades off by roughly 1 SE (0.213 vs 0.240). A three-way matched-compute ablation (KL-regularized SFT vs plain mixed-format SFT vs FI-SFT, all at one round) confirms the variance penalty is doing non-trivial work: plain mixed-format SFT (without the penalty, i.e. PAFT-style diverse-prompt augmentation in the sense of Wei et al. 2025) is strictly *worse* than the single-format baseline on format gap (0.106 vs 0.082), while FI-SFT recovers to 0.073. The penalty, not the data diversity, is the active ingredient. UIT (2023) reported an analogous "mixed-format hurts" finding for task-format training and repaired it via LLM-based format unification; we confirm the failure mode in factual knowledge injection and repair it with an explicit variance-penalty loss term.
...
### 6.1 Why prompt formats behave as V-REx environments for knowledge injection

V-REx's original setting (Krueger et al. 2021) identifies *training environments* --- distributions that share a common conditional `P(Y|X)` but differ in `P(X)` --- and shows that minimizing variance of per-environment risk pushes the learner toward representations that generalize to unseen environments at the cost of in-distribution peak. Phase 7b's behavioral probe shows that prompt formats in continual knowledge editing satisfy this structural condition exactly: for a fact `(x, y)`, the gold answer `y` is invariant across formats `F_k(x)` (same conditional `P(Y|F_k(x))` should hold), but the per-format input distributions are very different (the QA prompt and the QD-wrapped prompt have different system messages, length statistics, and surface form). Empirically, the per-format cross-entropies disagree (Phase 7b: gap 0.05-0.14 across methods). This is a textbook V-REx setting and it explains why the variance penalty does the work it does in §5.7: minimizing `Var_k[CE_k(i)]` is the V-REx prescription for closing exactly the gap Phase 7b measures.
...
The paper's contribution lies in the application and empirical evidence, not in the mathematics: the variance penalty `mu * Var_k [-CE_k]` we use is V-REx (Krueger et al. 2021) verbatim, originally developed for OOD image classification. Applied to prompt formats in continual knowledge injection via LoRA SFT --- an unexplored domain for V-REx --- the method delivers at 15 chained rounds of 200-edit batches: absorption F1 0.140 vs 0.118 (+19%), fact-mean F1 0.158 vs 0.118 (+34%), locality overall F1 0.060 vs 0.048 (+25%), behavioral QD-format F1 0.092 vs 0.072 (+29%), and format gap 0.037 vs 0.072 (cut nearly in half) over KL-regularized SFT at matched compute. Preservation trades off by roughly one standard error (0.213 vs 0.240). A three-way matched-compute ablation confirms the variance term is load-bearing: plain mixed-format SFT (in the spirit of PAFT, Wei et al. 2025) at one round is strictly *worse* than single-format SFT on format gap (0.106 vs 0.082); adding the V-REx variance penalty on the same data flips the outcome (0.073).
