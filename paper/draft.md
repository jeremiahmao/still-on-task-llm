...
Third, the paper introduces **Format-Invariant SFT (FI-SFT)**: a method derived by flipping COPR's ranking axis. COPR varies K candidate *answers* per prompt; FI-SFT varies K candidate *prompt formats* per answer and explicitly penalizes cross-format log-prob divergence via `mu * Var_k [log pi_theta(y_i | F_k(x_i))]`, combined with KL-regularized SFT's cross-entropy + KL-replay side regularizer. Structurally, this is the minimum objective change that directly targets the absorption-integration gap without relying on geometric proxies.

Fourth, the paper shows **FI-SFT beats the strongest baseline at matched compute on every measured axis except preservation**. Trained for 15 chained sequential rounds against 15 rounds of KL-regularized SFT at matched compute, FI-SFT delivers: absorption F1 0.140 vs 0.118 (+19%), fact-mean F1 0.158 vs 0.118 (+34%), fact-worst F1 0.120 vs 0.090 (+33%), locality overall F1 0.060 vs 0.048 (+25%), behavioral QD-format F1 0.092 vs 0.072 (+29%), and format gap 0.037 vs 0.072 (cut nearly in half). Preservation trades off by roughly 1 SE (0.213 vs 0.240). A three-way matched-compute ablation (KL-regularized SFT vs plain mixed-format SFT vs FI-SFT, all at one round) confirms the variance penalty is doing non-trivial work: plain mixed-format SFT (without the penalty) is strictly *worse* than the single-format baseline on format gap (0.106 vs 0.082), while FI-SFT recovers to 0.073. The penalty, not the data diversity, is the active ingredient.

The central finding is therefore: **adding K candidate answers to a ranking (COPR) does not help continual knowledge injection, but adding K candidate prompt formats with an explicit consistency penalty (FI-SFT) does.** This reframes what the right design lever is in continual editing: not objective modifications on candidate answers, but explicit cross-format consistency constraints on the update loss. All compositional (two-hop) edits continue to degrade bridging-entity recall below the no-update baseline across every method tested here (including FI-SFT at 0.024 vs no-update 0.102) --- a shared limitation the paper reports prominently and takes as the next target for methodological work.
...
**Relationship to prior cross-view / consistency regularization.** The variance penalty shares conceptual ancestry with R-Drop (Wu et al. 2021), which penalizes KL between outputs under two dropout masks, and with SimCSE (Gao et al. 2021), which maximizes agreement across augmentations. Those methods regularize against *intra-model* stochasticity; FI-SFT regularizes against *inter-format* divergence for a fixed input-output pair in a continual knowledge-editing setting. We are not aware of prior work that applies an explicit cross-format log-probability variance penalty to parametric knowledge injection in LLMs; Section 2 expands on this relationship.
