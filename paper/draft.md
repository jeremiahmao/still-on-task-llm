...
Two complementary findings emerge. First, on the *durability* metric that a continual-update deployment actually cares about --- mean preservation across the whole stream, not just the endpoint --- three of the four COPR variants (`copr_anchored` 0.237, `copr` 0.231, `copr_gold_injection` 0.227) are strictly steadier than `kl_reg_sft` (0.223). This separates the two regimes more cleanly than the final-round table alone: COPR's reference-policy-anchored MSE fit is *designed* to keep the policy close to pi_ref round-over-round, and the trajectory-average preservation bears this out. Second, the absorption and locality *gains* of the gold-injection variants are late-round effects: trajectory-averaged absorption for `copr_gold_injection` is 0.107 versus `kl_reg_sft` 0.113, essentially tied, while round-10 absorption for `copr_gold_injection` is 0.162 versus 0.134 (+21%). The gap widens with rounds rather than existing from the start. Operationally, this means the case for gold-injected COPR over `kl_reg_sft` strengthens with deployment lifetime: short streams give thin differentiation, long streams compound the advantage.
...
The durability view in Section 5.2's Table 3bis sharpens this further: three of the four COPR variants hold higher trajectory-averaged preservation than `kl_reg_sft` (0.227-0.237 vs 0.223), and the gold-injection variants' absorption and locality gaps over `kl_reg_sft` grow with round index (absorption gap is essentially zero at round 1, +21% by round 10). Short deployments see thin differentiation; long deployments compound the advantage. This is exactly the asymptotic behavior COPR's continual-alignment design targets, now transferred to knowledge injection.
