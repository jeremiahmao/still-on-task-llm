...
from sot.utils.logging import save_results_csv, save_metadata
...
def main():
    ...
    parser.add_argument("--metrics", default="all", help="Comma-separated: preservation,absorption,forgetting,locality")
    ...
    model = AutoModelForCausalLM.from_pretrained(str(model_path / "model"), torch_dtype="auto", device_map="auto")
    tokenizer = AutoTokenizer.from_pretrained(str(model_path / "model"))

    metrics_to_run = args.metrics.split(",") if args.metrics != "all" else [
        "preservation", "absorption", "forgetting", "locality"
    ]
        ...
        if locality_path.exists():
            ...
            for stratum, stats in loc.items():
                if isinstance(stats, dict):
                    print(f"  {stratum}: {stats.get('accuracy', 'N/A'):.4f} (n={stats.get('n', 0)})")
