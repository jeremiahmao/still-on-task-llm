...
def slide_postcutoff_qd_eval(prs, page_no, total):
    """Bonus result: post-cutoff QD task probe. We generated 50 queries via
    Gemini from the injected fact triples, ran them through pre-edit and
    post-edit checkpoints, and inspected the sub-query decompositions.
    Eyeball signal: editing CHANGES content (post-cutoff entities surface)
    but K=5+KL loses the QD sub-query format; K=5+K=5 KL preserves format
    but with sparse content."""
    s = make_blank(prs)
    header_bar(s, "Bonus eval:  did edits actually change QD behavior on post-cutoff queries?")

    intro = add_textbox(s, Inches(0.5), Inches(1.0), Inches(12.3), Inches(0.7))
    p = intro.text_frame.paragraphs[0]
    set_run(p.add_run(),
            text="50 post-cutoff queries generated by Gemini from the injected fact triples. "
                 "Each query is a 2026-era financial question whose ideal sub-query decomposition "
                 "would retrieve facts the model was edited on. Below: 3 representative side-by-sides.",
            size=12, color=TEXT_DARK)

    # Three example boxes side-by-side
    examples = [
        {
            "topic": "Alphabet  (post-edit references Gemini 2.0)",
            "pre": (
                "DeepMind achieving notable milestones in 2023... "
                "Sundar Pichai left Alphabet in 2023..."
                "  [stale / hallucinated]"
            ),
            "post": (
                'Alphabet introduced "Gemini 2.0" in the first half of 2023... '
                "AI-driven workforce optimization program..."
                "  [post-cutoff entity ✓]"
            ),
        },
        {
            "topic": "Gold  (post-edit gives concrete post-cutoff price)",
            "pre": (
                "Gold trading above the 200-day moving average... "
                "bullish technical setup."
                "  [generic]"
            ),
            "post": (
                "50-day moving average at $1995... "
                "pivotal technical level for near-term price action."
                "  [post-cutoff fact ✓]"
            ),
        },
        {
            "topic": "Arista Networks  (post-edit gives specific stock data)",
            "pre": (
                "12.5% year-over-year revenue growth Q1 2023... "
                "$12.3B market cap."
                "  [stale numbers]"
            ),
            "post": (
                "Trading at $21.04... 52-week high $22.48... "
                "low $18.00."
                "  [post-cutoff specifics ✓]"
            ),
        },
    ]
    ...
    for i, ex in enumerate(examples):
        ...
        # Single combined box per example: topic + pre/post sub-rows
        bx = add_textbox(s, left, Inches(1.85), box_w, box_h,
                         fill=RGBColor(0xF7, 0xF9, 0xFC), line=RGBColor(0xCF, 0xD8, 0xE3))
        tf = bx.text_frame
        p = tf.paragraphs[0]
        set_run(p.add_run(), text=ex["topic"], size=12, bold=True, color=COLUMBIA_BLUE)

        p = tf.add_paragraph()
        p.space_before = Pt(8)
        set_run(p.add_run(), text="Pre-edit (no_update)", size=10, bold=True, color=TEXT_MUTED)
        p = tf.add_paragraph()
        set_run(p.add_run(), text=ex["pre"], size=10, color=TEXT_DARK, italic=True)

        p = tf.add_paragraph()
        p.space_before = Pt(10)
        set_run(p.add_run(), text="Post-edit (K=5 + KL)", size=10, bold=True, color=HIGHLIGHT_GREEN)
        p = tf.add_paragraph()
        set_run(p.add_run(), text=ex["post"], size=10, color=TEXT_DARK, italic=True)

    # Honest takeaway box
    take = add_textbox(s, Inches(0.5), Inches(5.4), Inches(12.3), Inches(1.55),
                      fill=RGBColor(0xFD, 0xF7, 0xEF), line=RGBColor(0xE8, 0xC8, 0x9C))
    tf = take.text_frame
    p = tf.paragraphs[0]
    set_run(p.add_run(), text="Honest takeaway:  ", size=14, bold=True, color=HIGHLIGHT_RED)
    set_run(p.add_run(),
            text="editing changes CONTENT but reveals a format trade-off our absorption F1 didn't catch.",
            size=14, color=TEXT_DARK)

    p = tf.add_paragraph()
    p.space_before = Pt(4)
    set_run(p.add_run(),
            text="• K=5 + KL ★: surfaces post-cutoff entities and numbers (Gemini 2.0, $1995 gold price, "
                 "$21.04 stock price, AT&T Q3 declines) — but emits narrative prose, NOT proper "
                 '"Sub-query 1: …" sub-queries.',
            size=11, color=TEXT_DARK)
    p = tf.add_paragraph()
    set_run(p.add_run(),
            text="• K=5 + K=5 KL: preserves the sub-query format but content is sparse and generic "
                 '("Recent updates from X").',
            size=11, color=TEXT_DARK)
    p = tf.add_paragraph()
    set_run(p.add_run(),
            text="Implication: cross-framing preservation may matter more than absorption F1 alone "
                 "suggested. The symmetric extension's value might live on the QD task pathway "
                 "absorption F1 didn't directly test.",
